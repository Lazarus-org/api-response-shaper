...
class TestDynamicResponseMiddleware:
    ...
    def test_success_response(self, request_factory: RequestFactory, get_response: Callable) -> None:
        """
    ...
    def test_error_response(self, request_factory: RequestFactory, get_response: Callable) -> None:
        """
    ...
    def test_excluded_paths(self, request_factory: RequestFactory, get_response: Callable) -> None:
        """
        ...
        with patch.object(response_shaper_config, 'excluded_paths', new=['/api/excluded/']):
    ...
    def test_process_exception(self, request_factory: RequestFactory, get_response: Callable) -> None:
        """
        ...
        with patch.object(DynamicResponseMiddleware, '_build_error_response') as mock_build_error:
            ...
            response = middleware.process_exception(request, IntegrityError("Integrity Error"))
            response_data = self.parse_json_response(response)
            assert response.status_code == 400
            assert response_data == {
                'status': False,
    ...
    def test_custom_success_handler(self, request_factory: RequestFactory, get_response: Callable) -> None:
        """
        ...
        with patch('django.utils.module_loading.import_string', return_value=custom_success_handler):
            request = request_factory.get('/api/test/')
            middleware = DynamicResponseMiddleware(get_response)
    ...
    def test_custom_error_handler(self, request_factory: RequestFactory, get_response: Callable) -> None:
        """
        ...
        with patch('django.utils.module_loading.import_string', return_value=custom_error_handler):
            request = request_factory.get('/api/test/')
            middleware = DynamicResponseMiddleware(error_response)
    ...
    def test_process_object_does_not_exist_exception(self, request_factory: RequestFactory, get_response: Callable) -> None:
        """
        ...
        with patch.object(DynamicResponseMiddleware, '_build_error_response') as mock_build_error:
            ...
            response = middleware.process_exception(request, ObjectDoesNotExist("Object not found"))
            response_data = self.parse_json_response(response)
    ...
    def test_extract_complex_structure(self) -> None:
        ...
        """
        middleware = DynamicResponseMiddleware(None)
        errors = {"field1": [{"subfield": ["Subfield error", "Another error"]}, "Other error"], "field2": "Field2 error"}
        result = middleware._extract_first_error(errors)
        assert result == {"field2": "Field2 error"}

    def test_skip_non_json_content_type(self, request_factory: RequestFactory, get_response: Callable) -> None:
        """
    ...
    def test_process_json_response(self, request_factory: RequestFactory, get_response: Callable) -> None:
        """
    ...
    def test_process_exception_handling(self, request_factory: RequestFactory, get_response: Callable) -> None:
        """
    ...
    def test_process_validation_error(self, request_factory: RequestFactory, get_response: Callable) -> None:
        """
    ...
    def test_process_django_errors(self, request_factory: RequestFactory, get_response: Callable) -> None:
        """
