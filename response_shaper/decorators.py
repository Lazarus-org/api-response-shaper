...
def response_decorator(response_func: ResponseFuncType) -> Callable:
    """
    A base decorator that wraps a given view function and modifies its response
    based on the specified response function (e.g., api_response, paginated_api_response, etc.).
...
def format_api_response(func: Callable) -> Callable:
    """
    A decorator that applies the general API response format using `api_response`.
...
def format_paginated_response(func: Callable) -> Callable:
    """
    A decorator that applies the paginated API response format using `paginated_api_response`.
...
def format_error_response(func: Callable) -> Callable:
    """
    A decorator that applies the error API response format using `error_api_response`.
...
def format_minimal_success_response(func: Callable) -> Callable:
    """
    A decorator that applies the minimal success API response format using `minimal_success_response`.
...
def format_batch_response(func: Callable) -> Callable:
    """
    A decorator that applies the batch operation response format using `batch_api_response`.
...
def format_auth_response(func: Callable) -> Callable:
    """
    A decorator that applies the authentication response format using `auth_api_response`.
